...
import AlertProductionDelete from './Modal/AlertProductionDelete';
...
function ReactTable({ defaultColumns, defaultData }) {
    ...
    const handleSearch = (event) => {
        ...
        router.get(route('productions.index'), { search: event?.target.value }, { preserveState: true });
    };
    const handleChangePage = (event, newPage) => {
        setPage(newPage);
        router.get(route('productions.index'), { page: newPage }, { preserveState: true });
    };
    ...
    const reproductionRow = (draggedRowIndex, targetRowIndex) => {
        data.splice(targetRowIndex, 0, data.splice(draggedRowIndex, 1)[0]);
        router.post(route('productions.sort'), { productions: data }, { preserveState: true });
    };
            ...
            <TableContainer>
                <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between" sx={{ padding: 3, paddingTop: 0 }}>
                    ...
                    <Stack direction="row" alignItems="center" spacing={2}>
                        {auth.can.production_create ? (<Button variant="contained" startIcon={<Add />} onClick={() => {
                            router.get(route('productions.create'))
                        }} size="large">
                            Add Production
                        </Button>) : null}
                    </Stack>
...
export default function RowDragDrop({ productions }) {
    const { auth, ziggy } = usePage().props;
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <Stack direction="row" spacing={1} alignItems="center">
                    {auth.can.production_edit ? (<Tooltip
                        componentsProps={{
                            tooltip: {
                                sx: {
                    ...
                    >
                        <IconButton
                            color="primary"
                            onClick={() => {
                                router.get(route('productions.edit', { production: row.original?.id }));
                            }}
                        >
                            <Edit />
                        </IconButton>
                    </Tooltip>) : null}
                    {/* {auth.can.production_destroy ? (<Tooltip
                        componentsProps={{
                            tooltip: {
                                sx: {
                                    backgroundColor: mode === ThemeMode.DARK ? theme.palette.grey[50] : theme.palette.grey[700],
    ...
    return (
        ...
        >
            <Head title="Backoffice Production" />
            <MainCard title="Production">
                <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
                    <ReactTable {...{ defaultColumns, defaultData: productions }} />
                </DndProvider>
                <AlertProductionDelete id={Number(resourceDeleteId)} title={resourceDeleteTitle} open={open} handleClose={handleClose} />
            </MainCard>
        </AuthenticatedLayout>
    );
}
