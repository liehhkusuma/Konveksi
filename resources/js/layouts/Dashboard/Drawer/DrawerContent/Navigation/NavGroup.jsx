...
export default function NavGroup({
    ...
    const checkOpenForParent = (child, id) => {
        child.forEach((ele) => {
            ...
            if (ele.url && pathname.startsWith(ele?.pathname)) {
                setSelectedID(id);
            }
        });
    };
    const checkSelectedOnload = (data) => {
        ...
        childrens.forEach((itemCheck) => {
            ...
            if (itemCheck.url && pathname.startsWith(itemCheck?.pathname)) {
    ...
    const navCollapse = item.children?.map((menuItem, index) => {
        if (menuItem.access == '' || !auth.can[menuItem.access]) return null;
    ...
    const moreItems = remItems.map((itemRem, i) => (
        <Fragment key={i}>
            ...
            {itemRem?.elements?.map((menu) => {
                if (menu.access == '' || !auth.can[menu.access]) return null;
                switch (menu.type) {
                    case 'collapse':
                        return (
                            <NavCollapse
    ...
    const items = currentItem.children?.map((menu) => {
        if (!auth.can[menu.access]) return null;
        switch (menu?.type) {
            case 'collapse':
                return (
                    <NavCollapse
