...
import AlertMaterialCategoryDelete from './Modal/AlertMaterialCategoryDelete';
...
function ReactTable({ defaultColumns, defaultData }) {
    ...
    const handleSearch = (event) => {
        ...
        router.get(route('material-categories.index'), { search: event?.target.value }, { preserveState: true });
    };
    const handleChangePage = (event, newPage) => {
        setPage(newPage);
        router.get(route('material-categories.index'), { page: newPage }, { preserveState: true });
    };
    ...
    const rematerial_categoryRow = (draggedRowIndex, targetRowIndex) => {
        data.splice(targetRowIndex, 0, data.splice(draggedRowIndex, 1)[0]);
        router.post(route('material-categories.sort'), { material_categories: data }, { preserveState: true });
    };
            ...
            <TableContainer>
                <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between" sx={{ padding: 3, paddingTop: 0 }}>
                    ...
                    <Stack direction="row" alignItems="center" spacing={2}>
                        {auth.can.material_category_create ? (<Button variant="contained" startIcon={<Add />} onClick={() => {
                            router.get(route('material-categories.create'))
                        }} size="large">
...
export default function RowDragDrop({ material_categories }) {
    const { auth, ziggy } = usePage().props;
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <Stack direction="row" spacing={1} alignItems="center">
                    {auth.can.material_category_edit ? (<Tooltip
                        componentsProps={{
                            tooltip: {
                                sx: {
                    ...
                    >
                        <IconButton
                            color="primary"
                            onClick={() => {
                                router.get(route('material-categories.edit', { material_category: row.original?.id }));
                            }}
                        >
                            <Edit />
                        </IconButton>
                    </Tooltip>) : null}
                    {auth.can.material_category_destroy ? (<Tooltip
                        componentsProps={{
                            tooltip: {
                                sx: {
                                    backgroundColor: mode === ThemeMode.DARK ? theme.palette.grey[50] : theme.palette.grey[700],
    ...
    return (
        ...
        >
            <Head title="Backoffice Material Category" />
            <MainCard title="Material Category">
                <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
                    <ReactTable {...{ defaultColumns, defaultData: material_categories }} />
                </DndProvider>
                <AlertMaterialCategoryDelete id={Number(resourceDeleteId)} title={resourceDeleteTitle} open={open} handleClose={handleClose} />
            </MainCard>
        </AuthenticatedLayout>
    );
}
