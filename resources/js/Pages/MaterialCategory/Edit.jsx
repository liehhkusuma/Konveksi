...
export default function Create({ material_category }) {
    const { data, setData, patch, errors, processing, recentlySuccessful } =
        useForm({
            name: material_category.name,
            desc: material_category.desc,
            is_active: material_category.is_active ? true : false,
        });

    const handleSubmit = (e) => {
        e.preventDefault();

        patch(route('material-categories.update', { id: material_category.id }));
    };
    ...
    return (
        <AuthenticatedLayout
            ...
            <Head title="Edit Material Category" />
            <MainCard content={false} title="Edit Material Category" sx={{ '& .MuiInputLabel-root': { fontSize: '0.875rem' } }}>
                <form onSubmit={handleSubmit}>
                    ...
                    <Box sx={{ p: 2.5 }}>
                        <Stack direction="row" justifyContent="flex-end" alignItems="center" spacing={2} sx={{ mt: 2.5 }}>
                            <Link href={route('material-categories.index')}>
