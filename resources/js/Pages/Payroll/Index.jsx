...
import AlertPayrollDelete from './Modal/AlertPayrollDelete';
...
function ReactTable({ defaultColumns, defaultData }) {
    ...
    const handleSearch = (event) => {
        ...
        router.get(route('payrolls.index'), { search: event?.target.value }, { preserveState: true });
    };
    const handleChangePage = (event, newPage) => {
        setPage(newPage);
        router.get(route('payrolls.index'), { page: newPage }, { preserveState: true });
    };
    ...
    const repayrollRow = (draggedRowIndex, targetRowIndex) => {
        data.splice(targetRowIndex, 0, data.splice(draggedRowIndex, 1)[0]);
        router.post(route('payrolls.sort'), { payrolls: data }, { preserveState: true });
    };
            ...
            <TableContainer>
                <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between" sx={{ padding: 3, paddingTop: 0 }}>
                    ...
                    <Stack direction="row" alignItems="center" spacing={2}>
                        {auth.can.payroll_create ? (<Button variant="contained" startIcon={<Add />} onClick={() => {
                            router.get(route('payrolls.create'))
                        }} size="large">
                            Add Payroll
                        </Button>) : null}
                    </Stack>
...
export default function RowDragDrop({ payrolls }) {
    const { auth, ziggy } = usePage().props;
    ...
    const defaultColumns = [
        ...
        },
        {
            ...
            accessorKey: 'minus',
            cell: ({ row }) => {
                return formatter.format(row.original.minus);
            }
        },
        {
            id: 'cashbon',
            ...
            accessorKey: 'cashbon',
            cell: ({ row }) => {
                return formatter.format(row.original.cashbon);
            }
        },
        {
            id: 'tip',
            ...
            accessorKey: 'tip',
            cell: ({ row }) => {
                return formatter.format(row.original.tip);
            }
        },
        {
            id: 'cashbon',
                ...
                <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    >
                        <IconButton
                            color="primary"
                            onClick={() => {
                                router.get(route('payrolls.edit', { payroll: row.original?.id }));
                            }}
                        >
                            <Edit />
                        </IconButton>
                    </Tooltip>) : null}
                    {/* {auth.can.payroll_destroy ? (<Tooltip
                        componentsProps={{
                            tooltip: {
                                sx: {
                                    backgroundColor: mode === ThemeMode.DARK ? theme.palette.grey[50] : theme.palette.grey[700],
    ...
    return (
        ...
        >
            <Head title="Backoffice Payroll" />
            <MainCard title="Payroll">
                <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
                    <ReactTable {...{ defaultColumns, defaultData: payrolls }} />
                </DndProvider>
                <AlertPayrollDelete id={Number(resourceDeleteId)} title={resourceDeleteTitle} open={open} handleClose={handleClose} />
            </MainCard>
        </AuthenticatedLayout>
    );
}
