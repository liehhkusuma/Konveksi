...
import AlertConfigurationDelete from './Modal/AlertConfigurationDelete';
...
function ReactTable({ defaultColumns, defaultData }) {
    ...
    const handleSearch = (event) => {
        ...
        router.get(route('configurations.index'), { search: event?.target.value }, { preserveState: true });
    };
    const handleChangePage = (event, newPage) => {
        setPage(newPage);
        router.get(route('configurations.index'), { page: newPage }, { preserveState: true });
    };
    ...
    const reconfigurationRow = (draggedRowIndex, targetRowIndex) => {
        data.splice(targetRowIndex, 0, data.splice(draggedRowIndex, 1)[0]);
        router.post(route('configurations.sort'), { configurations: data }, { preserveState: true });
    };
            ...
            <TableContainer>
                <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between" sx={{ padding: 3, paddingTop: 0 }}>
                    ...
                    <Stack direction="row" alignItems="center" spacing={2}>
                        {auth.can.configuration_create ? (<Button variant="contained" startIcon={<Add />} onClick={() => {
                            router.get(route('configurations.create'))
                        }} size="large">
                            Add Configuration
                        </Button>) : null}
                    </Stack>
...
export default function RowDragDrop({ configurations }) {
    const { auth, ziggy } = usePage().props;
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <Stack direction="row" spacing={1} alignItems="center">
                    {auth.can.configuration_edit ? (<Tooltip
                        componentsProps={{
                            tooltip: {
                                sx: {
                    ...
                    >
                        <IconButton
                            color="primary"
                            onClick={() => {
                                router.get(route('configurations.edit', { configuration: row.original?.id }));
                            }}
                        >
                            <Edit />
                        </IconButton>
    ...
    return (
        ...
        >
            <Head title="Backoffice Apps Configuration" />
            <MainCard title="Apps Configuration">
                <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
                    <ReactTable {...{ defaultColumns, defaultData: configurations }} />
                </DndProvider>
                <AlertConfigurationDelete id={Number(resourceDeleteId)} title={resourceDeleteTitle} open={open} handleClose={handleClose} />
            </MainCard>
        </AuthenticatedLayout>
    );
}
