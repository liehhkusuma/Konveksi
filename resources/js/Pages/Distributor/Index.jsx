...
import AlertDistributorDelete from './Modal/AlertDistributorDelete';
...
function ReactTable({ defaultColumns, defaultData }) {
    ...
    const handleSearch = (event) => {
        ...
        router.get(route('distributors.index'), { search: event?.target.value }, { preserveState: true });
    };
    const handleChangePage = (event, newPage) => {
        setPage(newPage);
        router.get(route('distributors.index'), { page: newPage }, { preserveState: true });
    };
    ...
    const redistributorRow = (draggedRowIndex, targetRowIndex) => {
        data.splice(targetRowIndex, 0, data.splice(draggedRowIndex, 1)[0]);
        router.post(route('distributors.sort'), { distributors: data }, { preserveState: true });
    };
            ...
            <TableContainer>
                <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between" sx={{ padding: 3, paddingTop: 0 }}>
                    ...
                    <Stack direction="row" alignItems="center" spacing={2}>
                        {auth.can.distributor_create ? (<Button variant="contained" startIcon={<Add />} onClick={() => {
                            router.get(route('distributors.create'))
                        }} size="large">
                            Add Distributor
                        </Button>) : null}
                    </Stack>
...
export default function RowDragDrop({ distributors }) {
    const { auth, ziggy } = usePage().props;
    ...
    const defaultColumns = [
        ...
        },
        {
            ...
            accessorKey: 'name',
            cell: ({ row }) => {
                return row.original?.name + (row.original?.phone ? ` - ${row.original?.phone}` : '');
            }
        },
        {
            id: 'address',
            ...
            accessorKey: 'address',
            cell: ({ row }) => {
                return row.original?.address ? row.original.address : '-';
            }
        },
        {
            id: 'is_active',
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <Stack direction="row" spacing={1} alignItems="center">
                    {auth.can.distributor_edit ? (<Tooltip
                        componentsProps={{
                            tooltip: {
                                sx: {
                    ...
                    >
                        <IconButton
                            color="primary"
                            onClick={() => {
                                router.get(route('distributors.edit', { distributor: row.original?.id }));
                            }}
                        >
                            <Edit />
                        </IconButton>
                    </Tooltip>) : null}
                    {auth.can.distributor_destroy ? (<Tooltip
                        componentsProps={{
                            tooltip: {
                                sx: {
                                    backgroundColor: mode === ThemeMode.DARK ? theme.palette.grey[50] : theme.palette.grey[700],
    ...
    return (
        ...
        >
            <Head title="Backoffice Distributor" />
            <MainCard title="Distributor">
                <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
                    <ReactTable {...{ defaultColumns, defaultData: distributors }} />
                </DndProvider>
                <AlertDistributorDelete id={Number(resourceDeleteId)} title={resourceDeleteTitle} open={open} handleClose={handleClose} />
            </MainCard>
        </AuthenticatedLayout>
    );
}
