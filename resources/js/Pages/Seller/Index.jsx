...
import AlertSellerDelete from './Modal/AlertSellerDelete';
...
function ReactTable({ defaultColumns, defaultData }) {
    ...
    const handleSearch = (event) => {
        ...
        router.get(route('sellers.index'), { search: event?.target.value }, { preserveState: true });
    };
    const handleChangePage = (event, newPage) => {
        setPage(newPage);
        router.get(route('sellers.index'), { page: newPage }, { preserveState: true });
    };
    ...
    const resellerRow = (draggedRowIndex, targetRowIndex) => {
        data.splice(targetRowIndex, 0, data.splice(draggedRowIndex, 1)[0]);
        router.post(route('sellers.sort'), { sellers: data }, { preserveState: true });
    };
            ...
            <TableContainer>
                <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between" sx={{ padding: 3, paddingTop: 0 }}>
                    ...
                    <Stack direction="row" alignItems="center" spacing={2}>
                        {auth.can.seller_create ? (<Button variant="contained" startIcon={<Add />} onClick={() => {
                            router.get(route('sellers.create'))
                        }} size="large">
                            Add Seller
                        </Button>) : null}
                    </Stack>
...
export default function RowDragDrop({ sellers }) {
    const { auth, ziggy } = usePage().props;
    ...
    const defaultColumns = [
        ...
        },
        {
            ...
            accessorKey: 'balance',
            cell: ({ row }) => {
                return formatter.format(row.original.balance);
            }
        },
        {
            id: 'is_active',
                ...
                <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    >
                        <IconButton
                            color="primary"
                            onClick={() => {
                                router.get(route('sellers.edit', { seller: row.original?.id }));
                            }}
                        >
                            <Edit />
                        </IconButton>
                    </Tooltip>) : null}
                    {auth.can.seller_destroy ? (<Tooltip
                        componentsProps={{
                            tooltip: {
                                sx: {
                                    backgroundColor: mode === ThemeMode.DARK ? theme.palette.grey[50] : theme.palette.grey[700],
    ...
    return (
        ...
        >
            <Head title="Backoffice Seller" />
            <MainCard title="Seller">
                <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
                    <ReactTable {...{ defaultColumns, defaultData: sellers }} />
                </DndProvider>
                <AlertSellerDelete id={Number(resourceDeleteId)} title={resourceDeleteTitle} open={open} handleClose={handleClose} />
            </MainCard>
        </AuthenticatedLayout>
    );
}
