...
import AlertEmployeeDelete from './Modal/AlertEmployeeDelete';
...
function ReactTable({ defaultColumns, defaultData }) {
    ...
    const handleSearch = (event) => {
        ...
        router.get(route('employees.index'), { search: event?.target.value }, { preserveState: true });
    };
    const handleChangePage = (event, newPage) => {
        setPage(newPage);
        router.get(route('employees.index'), { page: newPage }, { preserveState: true });
    };
    ...
    const reemployeeRow = (draggedRowIndex, targetRowIndex) => {
        data.splice(targetRowIndex, 0, data.splice(draggedRowIndex, 1)[0]);
        router.post(route('employees.sort'), { employees: data }, { preserveState: true });
    };
            ...
            <TableContainer>
                <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between" sx={{ padding: 3, paddingTop: 0 }}>
                    ...
                    <Stack direction="row" alignItems="center" spacing={2}>
                        {auth.can.employee_create ? (<Button variant="contained" startIcon={<Add />} onClick={() => {
...
export default function RowDragDrop({ employees }) {
    const { auth, ziggy } = usePage().props;
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <Stack direction="row" spacing={1} alignItems="center">
                    {auth.can.employee_edit ? (<Tooltip
                        componentsProps={{
                            tooltip: {
                                sx: {
                    ...
                    >
                        <IconButton
                            color="primary"
                            onClick={() => {
                                router.get(route('employees.edit', { employee: row.original?.id }));
                            }}
                        >
                            <Edit />
                        </IconButton>
                    </Tooltip>) : null}
                    {auth.can.employee_destroy ? (<Tooltip
                        componentsProps={{
                            tooltip: {
                                sx: {
                                    backgroundColor: mode === ThemeMode.DARK ? theme.palette.grey[50] : theme.palette.grey[700],
    ...
    return (
        ...
        >
            <Head title="Backoffice Employee" />
            <MainCard title="Employee">
                <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
                    <ReactTable {...{ defaultColumns, defaultData: employees }} />
                </DndProvider>
                <AlertEmployeeDelete id={Number(resourceDeleteId)} title={resourceDeleteTitle} open={open} handleClose={handleClose} />
            </MainCard>
        </AuthenticatedLayout>
    );
}
